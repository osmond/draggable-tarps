...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (window.firebase && firebase.database) {
    ...
    firebaseRef.on('child_added', (snapshot) => {
      ...
      if (!exists) {
        displaySuggestion(text, undefined, false, suggestMessagesContainer);
        addSuggestionItem(text, time, suggestList, firebaseRef);
  ...
  loadSuggestions().forEach((s) => {
    if (s && s.text) {
      addSuggestionItem(s.text, s.time, suggestList, firebaseRef);
    }
  });
